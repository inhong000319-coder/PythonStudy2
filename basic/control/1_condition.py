...
n2 = int(input("나이를 입력해주세요 : "))
if n2 >= 0:
    if n2 >= 19:
        print(f"{n2}세 성인입니다.")
    else:
        print(f"{n2}세 미성년자 입니다.")
else:
    print("나이는 음수가 될수 없습니다.")
    '''
...
'''
    ...
    * import random 추가해야함
    ...
    random.random() : 0 이상 1미만의 실수값을 랜덤하게 리턴 (Math.random() 유사)
...
if user in rps: # 리스트(rps)내에 데이터(user) 가 있으면 True, 없으면 False
    ...
    if rps.index(user) >= 0:        #가위, 바위, 보 중 선택한 경우
