...
def practice_1d():
  ...
  print('앞의 3개 요소 추출:')
  print(arr[:3:])   # => 0,1,2번 인덱스에 해당하는 요소 추출. 시작 인덱스 생략
...
def practice_2d():
  ...
  print(arr[:2, 2:])  # 행 : 0번부터 1번 인덱스까지 / 열 : 2번부터 끝 인덱스까지

  print('배열에서 1,2,3,4 를 추출 : ')
  print(arr[:1,:])   # =>  print(arr[0])  

  print('배열에서 2,6,10을 추출 : ')
  print(arr[:3,1:2])    # => print(arr[:, 1])
...
def practice_3d():
  ...
  print('** indexing **')
  print(' 데이터 중 6을 출력 : ')
  print(arr[:1, 1:,2:])   # => print(arr[0, 1, 2])
...
def practice_boolean_indexing():
  ...
  # * 50 < x < 80 데이터만 추출
  result = arr[(arr > 50) & (arr < 80)]
  # => 각 조건은 소괄호로 묶어주고, AND 연산은 & 기호를 사용해야 함
  print(result)
  print(result[0])
